  ...
  server.getRecords().then(function(){
    ...
    server.config.addresses.forEach(function(address){
      server.config.ports.forEach(function(port){
        server.createServer(port, address);
      });
    });
...
Server.prototype.getRecord = function(host){
  ...
  return Q.fcall(function(){
    ...
    if(!record || !record.ports || !record.ports[port]){
      throw 'Subdomain not configured';
    }
    return record.ports[port].alias ? server.getRecord(record.ports[port].alias) : record.ports[port];
  }).then(function(record){
    return record;
  });
};
...
function selectRandomTarget(targets){
  ...
  try{
    for(host in targets){
      totalWeight += parseInt(targets[host], 10);
    }
    random = Math.random() * totalWeight;
    
    for(host in targets){
      if(random < parseInt(targets[host], 10)) return host;
      random -= parseInt(targets[host], 10);
    }
