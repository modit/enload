...
Server.prototype.getRecords = function(){
  ...
  server.watcher = server.etcd.watcher(server.config.etcd.directory, null, { recursive: true, consistent: true });
  ...
  server.watcher.on('expire', function(change){
    removeRecord(change.node);
  });
    
  return server.etcd.getQ(server.config.etcd.directory, { recursive: true, consistent: true }).spread(function(result){
