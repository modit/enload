...
Server.prototype.setActivity = function(key){
  var server  = this;
  var now     = Date.now();
  
  server.etcd.getQ(key).spread(function(result){
    return (now - result.node.value) > (process.env.ACTIVE_THROTTLE || 3000);
  }).catch(function(){
    return true;
  }).then(function(write){
    if(write){
      server.etcd.setQ(key, now, { ttl: process.env.ACTIVE_TTL || 21600 });
    }
  });
};
...
Server.prototype.getTarget = function(ip, host, port){
  ...
  return Q.fcall(function(){
    ...
    if(record.redirect){
      ...
      var target = selectTarget(ip, record.targets);
      
      server.setActivity(server.config.etcd.directory + '/active-targets/' + target);
...
Server.prototype.getRecords = function(){
  ...
  function addRecord(change, root){
    if(change.node.key.indexOf('/' + server.config.etcd.directory + '/domains') === 0){
      console.log('Adding domain record', change.node.key);
    }
  ...
  server.stateWatcher = server.etcd.stateWatcher(server.config.etcd.directory, { recursive: true, consistent: true });
  
  server.stateWatcher.on('state', function(result){
    console.log('Setting Records');
    server.records.domains = {};
    addRecord(result);
  });
  
  server.stateWatcher.on('change', function(change){
  ...
  server.stateWatcher.on('expire', function(change){
    removeRecord(change.node);
  });
};
