...
function StateWatcher(etcd, path, options){
  var self = this, timeout, heartbeat;
  ...
  (function init(){
    ...
    etcd.getQ(path, options).spread(function(result, headers){
      ...
      var watcher = etcd.watcher(path, parseInt(headers['x-etcd-index']) + 1, JSON.parse(JSON.stringify(options))); //JSON poor man's copy
      ...
      ['change', 'expire', 'stop', 'error'].forEach(function(event){
        watcher.on(event, function(data){
          if(data.node.key.indexOf('$$heartbeat') === -1){
      ...
      if(result.node.dir){
        heartbeat = setInterval(function(){
          etcd.setQ(path + '/$$heartbeat', Date.now(), { ttl: 0 }).catch(function(error){
            console.warn(error);
            clearInterval(heartbeat);
          });
        }, 45000);
